...
##selects a reference feature that spatially intersects the input feature and transferes specified values
...
#Initialize UpdateCursor on update_feature to update its values called in the cursor below. Call 'SHAPE@' to reference the feautre in space for select by location.
with arcpy.da.UpdateCursor(update_feature, ['PIN','MapBlockLo','SHAPE@']) as cursor:
    ...
    for row in cursor:
        ...
        mapblo = str(row[1])
        feature = row[2]
        #If the lenght of the pin value is > 1, its is already populated. Remove if condition should the value be updated regardless of the presence of a preexisting value.
        if len(pin) > 1:
            continue
        else:

...
#Initialize a number to append to the end of file name 'dir_iterator' ensuring a file is not overwritten in the event of more than one survey of a particular entity.
unique_ID = 0

#Initialize UpdateCursor on Update_FC to populate the Photo field with paths to the image's repsective directory 'fileloc' using values from the fields called below.
...
with arcpy.da.UpdateCursor(Update_FC, ['GlobalID_Static','St_Direc','St_Name','St_Type','St_Num','AcctID','Photo']) as cursor_1:
    for row_1 in cursor_1:
        ...
        dir_iterator = str(strt_direc) + '_' + str(strt_name) + '_' + str(strt_type) + '_' + str(strt_num) + '_' + str(locationID) + '_' + str(project_code) + '_' + str(unique_ID)

        #Query the rows in Attach_table that are associated with row_1 in Update_FC and name queried table 'table_view_' + the GlobalID_Static.
        ...
        with arcpy.da.UpdateCursor("table_view_" + str(row_1[0]), ['ATT_NAME']) as cursor_2:
            for row_2 in cursor_2:
                #Set the old value in Attach_table to the newly derived value plus the old value. Execute the update.
        ...
        #Get this directory as a value 'fileloc' to populate row_1 in Update_FC as the location of the file for reference. Execute the update.
